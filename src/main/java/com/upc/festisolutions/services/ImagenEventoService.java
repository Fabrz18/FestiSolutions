...
public class ImagenEventoService implements IImagenEventoService {
    ...
    public List<ImagenEventoDTO> listar(){
        ...
        return imagenEventos.stream().map(imagenevento  -> modelMapper.map(imagenevento, ImagenEventoDTO.class)).toList();
    }
    @Override
    public List<ImagenEventoDTO> buscar(Integer id){
        List<Imagenevento> imagenEventos = imagenEventoRepository.findAllByEvento_Id(id);
        return imagenEventos.stream().map(imagenevento -> modelMapper.map(imagenevento, ImagenEventoDTO.class)).toList();
    }
}
