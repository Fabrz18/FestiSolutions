...
public class ResenaEventoService implements IResenaEventoService {
    ...
    public List<ResenaEventoDTO> listarResenaEvento() {
        ...
        return resenas.stream()
                .filter(resena -> resena.getEvento() != null && !resena.getEvento().getEstado().equals("Eliminado"))
    ...
    public List<ResenaEventoDTO> listarResenaEventoSegunEvento(Integer idEvento) {
        List<Resenaevento> resenaEventos = resenaEventoRepository.encontrarResenas(idEvento);
        return resenaEventos.stream().map(resena -> modelMapper.map(resena, ResenaEventoDTO.class)).toList();
    }

    @Override
    public List<ResenaEventoDTO> listarResenaEventoSegunAnfitrionYEvento(Integer idAnfitrion, Integer idEvento) {
        List<Resenaevento> resenaEventos = resenaEventoRepository.findAllByAnfitrion_IdAndEvento_Id(idAnfitrion, idEvento);
        return resenaEventos.stream().map(resena -> modelMapper.map(resena, ResenaEventoDTO.class)).toList();
    }
}
